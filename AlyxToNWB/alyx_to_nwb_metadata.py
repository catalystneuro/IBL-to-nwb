...
class Alyx2NWBMetadata:
    ...
    def nwbfile_metadata(self):
        ...
        nwbfile_metadata_dict['NWBFile'].update(
            ...
            identifier=self.eid,
            institution=[i['institution'] for i in self.lab_table
                         if i['name'] == [self.eid_session_info['lab']][0]][0],
            lab=self.eid_session_info['lab'],
            protocol=self.eid_session_info['task_protocol'],
            notes='Procedures:' + ','.join(
                self.eid_session_info['procedures']) + ', Project:' + self.eid_session_info['project'],
            session_description=self.eid_session_info['narrative'])
    ...
    def subject_metadata(self):
        ...
        if sub_table_dict:
            subject_metadata_dict['IBLSubject'].update(
                ...
                date_of_birth=self._get_datetime(sub_table_dict.pop('birth_date'), format='%Y-%m-%d')
            )

            subject_metadata_dict['IBLSubject'].update(sub_table_dict)
            subject_metadata_dict['IBLSubject']['weighings'] = [str(i) for i in
                                                                subject_metadata_dict['IBLSubject']['weighings']]
            subject_metadata_dict['IBLSubject']['water_administrations'] = [str(i) for i in
                                                                            subject_metadata_dict['IBLSubject'][
