...
def main():
    ...
    try:
        ...
        with open(config_file, "w") as f:
            f.write(f"# IBL Conversion Configuration\n")
            f.write(f"# Generated by configure_networking.py\n")
            f.write(f"#\n")
            f.write(f"# WARNING: This file contains secrets (DANDI_API_KEY)\n")
            f.write(f"# DO NOT COMMIT TO GIT - File is gitignored for safety\n")
            f.write(f"#\n")
            f.write(f"# Network IDs (safe, not secrets):\n")
            f.write(f"VPC_ID={vpc_id}\n")
            f.write(f"SUBNET_ID={subnet_id}\n")
            f.write(f"SECURITY_GROUP_ID={sg_id}\n")
            f.write(f"REGION={REGION}\n")
            f.write(f"\n")
            f.write(f"# Secret (NEVER commit to git):\n")
            if existing_dandi_key:
                f.write(f"DANDI_API_KEY={existing_dandi_key}\n")
            else:
                f.write(f"DANDI_API_KEY=your-dandi-api-key-here\n")
        ...
        if not existing_dandi_key:
            ...
            print(f"   # Change: DANDI_API_KEY=your-dandi-api-key-here")
            print()
        print("Next Steps:")
        print(f"  1. Generate assignments: python generate_assignments.py --num-shards 3 --stub-test")
        print(f"  2. Launch instances:     python launch_ec2_instances.py --num-instances 3 --stub-test")
        print()
