...
with open(output_csv, "w", newline="") as csvfile:
    ...
    for idx, row in tqdm(bwm_df.iterrows(), total=len(bwm_df)):
        ...
        has_alf_files = all(any(pattern in str(d) for d in datasets) for pattern in histology_patterns)
...
print("\nHistology files:")
print(
    f"  Has files: {histology_qc_df['has_histology_files'].sum()} ({100*histology_qc_df['has_histology_files'].sum()/len(histology_qc_df):.1f}%)"
)
print(
    f"  Missing:   {(~histology_qc_df['has_histology_files']).sum()} ({100*(~histology_qc_df['has_histology_files']).sum()/len(histology_qc_df):.1f}%)"
)
