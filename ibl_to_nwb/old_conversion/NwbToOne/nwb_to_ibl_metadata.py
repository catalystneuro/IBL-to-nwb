...
class NWBToIBLSession:
    ...
    def _build_sessions_table(self):
        ...
        if self.nwbfile.lab_meta_data.get("Ibl_session_data"):
            nwb_data.update(
                nwb_to_ibl_dict(self.nwbfile.lab_meta_data.get("Ibl_session_data").fields, field_map_session_data)
            )
        # adding subject name:
        ...
        nwb_data["n_correct_trials"] = (
            sum(self.nwbfile.trials["choice"].data[()] != 0)
            if self.nwbfile.trials.get("choice")
            else len(self.nwbfile.trials)
        )
        nwb_data["narrative"] = ""
        # adding probes:
        count = 0
        ...
        for i, j in self.nwbfile.devices.items():
            if isinstance(j, IblProbes):
                ...
                nwb_data["probe_insertion"][count]["trajectory_estimate"] = [json.loads(ii) for ii in temp]
