    ...
    def _get_multiple_data(self, datastring):
        ...
        if not ((spike_cluster_data is None) | (spike_cluster_data is None)):  # if bot hdata are found only then
            ...
            if not self.data_attrs_dump.get(
                "unit_table_length"
            ):  # if unit table length is not known, ignore spike times
                return None
            ...
            for j, i in enumerate(i_loop):
                ...
                ls_grouped = [[np.nan]] * self.data_attrs_dump["unit_table_length"][
                    j
                ]  # default spiking time for clusters with no time
    ...
    def _load_as_array(self, dataset_to_load: str, dataset_key: str, loaded_dataset_: SessionDataInfo):
        ...
        if all([dtype in [".csv", ".npy", ".json"] for dtype in datatype]):  # csv is for clusters metrics
            ...
            if dataset_to_load.split(".")[0] in ["_iblqc_ephysSpectralDensity", "_iblqc_ephysTimeRms", "ephysData"]:
                self.data_attrs_dump[dataset_to_load] = [i.name.split(".")[0] + "_" + i.parent.name for i in dataloc]
            ...
            if dataset_to_load.split(".")[0] in ["camera"]:
                ...
                if not self.data_attrs_dump.get(dataset_to_load):
                    self.data_attrs_dump[dataset_to_load] = [
                        i.split(".")[0] for i in datanames_sorted[: int(len(datanames_sorted))]
                    ]
                ...
                for no, fields in enumerate(loaded_dataset_sorted[:filetype_change_id]):
                    df = pd.DataFrame(
                        data=loaded_dataset_sorted[no + len(loaded_dataset_sorted) // 2],
                        columns=loaded_dataset_sorted[no]["columns"],
                    )
                    df_out.append(df)
                return df_out
            if (
                "audioSpectrogram.times" in dataset_to_load or "trials.interval" in dataset_to_load
            ):  # ignore trials.intervals_bpod if present
                return loaded_dataset_[0] if isinstance(loaded_dataset_, list) else loaded_dataset_
            if (
                not self.data_attrs_dump.get("unit_table_length") and "cluster" in dataset_to_load
            ):  # capture total number of clusters for each probe, used in spikes.times
                self.data_attrs_dump["unit_table_length"] = [loaded_dataset_[i].shape[0] for i in range(self.no_probes)]
            if (
                not self.data_attrs_dump.get("electrode_table_length") and "channel" in dataset_to_load
            ):  # capture total number of clusters for each probe, used in spikes.times
                self.data_attrs_dump["electrode_table_length"] = [
                    loaded_dataset_[i].shape[0] for i in range(self.no_probes)
                ]
            ...
            if isinstance(self.nwb_metadata["NWBFile"]["session_start_time"], datetime):
                ...
                dt_start = datetime.strptime(self.nwb_metadata["NWBFile"]["session_start_time"], "%Y-%m-%d %X").replace(
                    tzinfo=get_localzone()
                )
            dt_start = dt_start.replace(tzinfo=None)
            dt_func = lambda x: ((datetime.strptime(x[:26], "%Y-%m-%dT%H:%M:%S.%f")) - dt_start).total_seconds()
